...
class Ender(Generales):
    def ender_chest(self, msg = '', text = '', datos = None):
        self.config()
        ...
        if not numero_busqueda and numero_item != 0:
            return msg.reply(f'Revisa que el numero de item existe con "{self.prefijo}ec {username}"')
                ...
                if nombre != 'SkyBlock Menu (Right Click)':
                    if numero_items == aux:
                        ...
                        try:
                            ...
                            pass
                        pagina_actual = cantidad_original // elementos_per_page + 1
                        if cantidad_original % 45 == 0:
                            pagina_actual -= 1
                        if detallado:
                            inventario += f'{nombre}\n'
                            ...
                            inv_full.append([f'{numero_items}.- {nombre}\n', cantidad_original])
                    numero_items += 1

        # -------------- Regresando para Interior ----------------
        if interno:
            return full_datos

        # -------------- Regresando Inventario ----------------
        if not detallado:
            color = self.color_aleatorio()
            description = ''
            pagina = 1
            for i in range(len(inv_full)):
                description += f'{inv_full[i][0]}'
                if inv_full[i][1] // elementos_per_page > pagina - 1:
                    description = f'Pagina {pagina}\n{description}'
                    embed = discord.Embed(title = f"{username}'s Ender Chest en {perfil_name}", description = description, color = color)
                    embed.set_thumbnail(url=f'https://crafatar.com/renders/body/{uuid}?size=40')
                    text = f'Pagina {pagina}'
